...
export async function invokeLLM(
  params: InvokeParams,
  options?: { provider?: "ollama" | "anthropic" | "forge" }
): Promise<InvokeResult> {
  const provider = options?.provider || ENV.llmProvider;

  // If provider override specified, use it directly without fallback
  if (options?.provider) {
    return await invokeByProvider(provider, params);
  }

  // For Ollama, try local first then fallback to Anthropic
  if (provider === "ollama") {
    try {
      ...
      if (isConnectionError && ENV.anthropicApiKey) {
...
async function invokeByProvider(provider: string, params: InvokeParams): Promise<InvokeResult> {
  switch (provider) {
    case "ollama":
      return await invokeOllama(params);
    case "anthropic":
